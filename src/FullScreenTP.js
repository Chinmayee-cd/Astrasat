...
class SummaryTP extends Component {
  modalRef = null;
  btnRef = null;
  modalState = "";
  action = "";
  ...
  componentDidMount() {
    ...
    d3.csv(data, (d) => ({
      ...
      const xScale = d3
        ...
        .range([0, width]);
      var temp_max = d3.max(dataset, (d) => d.temp);
      var pressure_max = d3.max(dataset, (d) => d.pressure);
...
class FullScreenTP extends Component {
  modalRef = null;
  btnRef = null;
  modalState = "";
  action = "";
  ...
  componentDidMount() {
    ...
    d3.csv(data, (d) => ({
      ...
      const yScale = d3
        .scaleLinear()
        .domain([0, d3.max(slicedData, (d) => d[max_parameter])])
        .range([height, 0]);
